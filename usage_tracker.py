...
class S3LogParser(object):
	def __init__(self):
		self.format = r'%{owner}i %{bucket}i %{datetime}t %{ip}h %{requester}i %{requestid}i %{operation}i %{key}i \"%{requesturi}i\" %{status}s %{error}i %{bytes}b %{objectsize} %{totaltime}i %{turnaround}i \"%{referer}i\" \"%{useragent}i\" %{versionid}i'
...
class S3LogGrabber(object):
	def __init__(self,log_bucket, log_prefix, s3conn, max_keys=10000):
	...
	def logiter(self,marker=''):
		...
		def fetchlogiter(listed):
			...
			for l in listed.entries:
				...
				logobj = self.conn.get(self.bucket,l.key)
		...
		listed = self.conn.list_bucket(self.bucket,options)
...
class S3LogProcessor(object):
	def __init__(self,log_bucket, log_prefix, s3conn, mongodb):	
		self.bucket = log_bucket
		self.conn = s3conn
		self.prefix = log_prefix
		self.db = mongodb
		
		self.log_grabber = S3LogGrabber(self.bucket,self.prefix, self.conn)
		...
		try:
			for logline in self.log_grabber.logiter(marker_key):
				...
				if plogline['operation'] == 'REST.GET.OBJECT':
					...
					# We should have already recorded usage to the pile when it was first uploaded
